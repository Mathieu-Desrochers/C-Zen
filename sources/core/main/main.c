...
//  customer_row_free(customer_row);
  ...
  {
    int customers_table_delete_result = customers_table_delete(sql_connection, customer_row);

    check(customers_table_delete_result == 0, "customers_table_delete_result: %d",
      customers_table_delete_result);
